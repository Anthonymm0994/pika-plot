...
use datafusion::arrow::array::{StringArray, Int64Array, Float64Array};
use datafusion::arrow::datatypes::{DataType, Field, Schema};
use datafusion::arrow::record_batch::RecordBatch;
use std::sync::Arc;

fn main() {
    // Create test data with two groups that are identical except for one column
    let group_ids = vec!["group1", "group1", "group1", "group1", "group2", "group2", "group2", "group2"];
    let names = vec!["Alice", "Bob", "Alice", "Bob", "Alice", "Bob", "Alice", "Bob"];
    let ages = vec![25, 30, 25, 30, 25, 30, 25, 30];
    let jobs = vec!["Engineer", "Manager", "Engineer", "Manager", "Engineer", "Manager", "Engineer", "Manager"];
    let dates = vec!["2023-01-01", "2023-01-02", "2023-01-03", "2023-01-04", "2023-01-05", "2023-01-06", "2023-01-07", "2023-01-08"]; // Different dates
    ...
    // Test 1: Detect duplicates ignoring the date column
    println!("=== Test 1: Ignoring date column ===");
    ...
    if result.total_duplicates > 0 {
        ...
        let output_path = std::path::Path::new("test_duplicates/clean_test_table.arrow");
