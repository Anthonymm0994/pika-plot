...
impl PlotTrait for ViolinPlot {
    ...
    fn render(&self, ui: &mut Ui, data: &PlotData, config: &PlotConfiguration) {
        ...
        let violin_config = ViolinPlotConfig::default(); // TODO: Add to PlotSpecificConfig
        ...
        plot.show(ui, |plot_ui| {
            // Render each violin
            for (i, (group, values, stats)) in violin_data.iter().enumerate() {
                let x_pos = i as f64;
                let color = data.series[i].color;
                
                // Calculate KDE for violin shape
                let kde_points = self.calculate_kde(values, violin_config.bandwidth, 100);
                ...
                if violin_config.show_box_plot {
                    // Create a simple box plot manually since BoxSpread API might have changed
                ...
                plot_ui.text(egui_plot::Text::new(
                    EguiPlotPoint::new(x_pos, stats.min - (stats.max - stats.min) * 0.05),
